...
void DrawTH1(TFile* dataFile, const char *nameHisto, const char *XTitle, const char *YTitle, bool log, double xmin, double xmax){
  ...
  if( !h1 ){
    std::cout<< std::endl;
    std::cout<< "Histogram doesn't found: "<< nameHisto << std::endl;
    return;
  } 
  ...
  std::cout<< "Saved: "<< HistoName << std::endl;
  if(pdfsave)  canvas ->SaveAs("Plot/"+HistoName+".pdf");
  if(rootsave)  canvas ->SaveAs("Plot/"+HistoName+".root");
  canvas ->Destructor();
}
//////////////////////////////////////
...
*/
  ...
  canvas ->SaveAs("Plot/"+HistoName+".png");
  std::cout<< "Saved: "<< HistoName << std::endl;
  if(pdfsave)  canvas ->SaveAs("Plot/Fit_"+HistoName+".pdf");
  if(rootsave)  canvas ->SaveAs("Plot/Fit_"+HistoName+".root");
  canvas ->Destructor();
}
///////////////////////////////////
void LinearFit(TFile* dataFile, const char *nameHisto, const char *XTitle, const char *YTitle, bool log, double xmin, double xmax){
  
  TH1F *h1 = (TH1F*)dataFile->Get(nameHisto );
  if( !h1 ) 
    TProfile *h1 = (TProfile*)dataFile->Get(nameHisto );
  ...
  // linear->SetRange(xmin, xmax);

  // h1 ->Fit(linear, "R");
  h1 ->Fit(linear,"Q");

  double q_linear       =  linear->GetParameter(0);
  //  double qErr   = linear->GetParError(0);
  double m_linear       =  linear->GetParameter(1);
  //  double mErr  = linear->GetParError(1);
  
  //  std::cout<<"coeff angolare "<< m_linear<<" +- "<<mErr<<std::endl;
  //  std::cout<<"intercetta "<< q_linear<<" +- "<<qErr<<std::endl;
  //  std::cout<<"ChiSquare/NDF "<<linear->GetChisquare()<<" / "<<linear->GetNDF()<<std::endl;
  
  TPaveText* fitlabel = new TPaveText(0.60, 0.70, 0.80, 0.80, "brNDC");
  fitlabel->SetTextSize(0.03);
  fitlabel->SetFillColor(0);
  TString TextFit = TString::Format("%.4f + x*(%.4f)", q_linear, m_linear);
  //  TString Text_Sigma = TString::Format("Sigma: %.4f #pm %.4f", sigmaFit, sigmaFitErr);
  fitlabel->AddText(TextFit);
  //  fitlabel->AddText(Text_Sigma);
  ...
  std::cout<< "Saved: Fit_"<< HistoName << std::endl;
  if(pdfsave)  canvas ->SaveAs("Plot/Fit_"+HistoName+".pdf");
  if(rootsave)  canvas ->SaveAs("Plot/Fit_"+HistoName+".root");
  canvas ->Destructor();
}
...
void LinearFit(TFile* dataFile1, TFile* dataFile2, const char *nameHisto, const char *XTitle, const char *YTitle, bool log, double xmin, double xmax){
  
  TH1F *h1 = (TH1F*)dataFile1->Get(nameHisto);
  if( !h1 ) 
    TProfile *h1 = (TProfile*)dataFile1->Get(nameHisto);

  TH1F *h2 = (TH1F*)dataFile2->Get(nameHisto);
  if( !h2 ) 
    TProfile *h2 = (TProfile*)dataFile2->Get(nameHisto);
  ...
  TF1* linear1 = new TF1( "linear1", "[0]+x*[1]" );  
  TF1* linear2 = new TF1( "linear2", "[0]+x*[1]" );  
  //  gaussian->SetParameter(0, h1 ->GetMaximum());
  //  gaussian->SetParameter(1, histMean);
  //  linear->SetRange(xmin, xmax);
  // h1 ->Fit(linear, "R");
  ...
  double q_linear1       =  linear1->GetParameter(0);
  //  double qErr   = linear->GetParError(0);
  double m_linear1      =  linear1->GetParameter(1);
  //  double mErr  = linear->GetParError(1);
  
  double q_linear2       =  linear2->GetParameter(0);
  double m_linear2      =  linear2->GetParameter(1);

  //  std::cout<<"coeff angolare "<< m_linear<<" +- "<<mErr<<std::endl;
  //  std::cout<<"intercetta "<< q_linear<<" +- "<<qErr<<std::endl;
  //  std::cout<<"ChiSquare/NDF "<<linear->GetChisquare()<<" / "<<linear->GetNDF()<<std::endl;
  
  TPaveText* fitlabel1 = new TPaveText(0.30, 0.80, 0.60, 0.90, "brNDC");
  fitlabel1->SetTextSize(0.03);
  fitlabel1->SetTextColor(kBlue);
  fitlabel1->SetFillColor(0);
  TString TextFit1 = TString::Format("%.4f + x*(%.4f)", q_linear1, m_linear1);
  fitlabel1->AddText(TextFit1);

  TPaveText* fitlabel2 = new TPaveText(0.60, 0.80, 0.90, 0.90, "brNDC");
  fitlabel2->SetTextSize(0.03);
  fitlabel2->SetTextColor(kRed);
  fitlabel2->SetFillColor(0);
  TString TextFit2 = TString::Format("%.4f + x*(%.4f)", q_linear2, m_linear2);
  fitlabel2->AddText(TextFit2);
...
void DrawComparison(TFile* dataFile, const char *nameHisto, const char *XTitle, const char *YTitle, bool log, bool colz, double xmin, double xmax){
  ...
  if( !h1 ){
    std::cout<< std::endl;
    std::cout<< "Histogram doesn't found: "<< nameHisto <<"HLT"<< std::endl;
    return;
  } 
  if( !h2 ){
    std::cout<< std::endl;
    std::cout<< "Histogram doesn't found: "<< nameHisto <<"Reco"<< std::endl;
    return;
  } 
...
//////////////////////////////////////
void DrawComparisonNormalized(TFile* dataFile1, TFile* dataFile2, const char *nameHisto, const char *XTitle, const char *YTitle, bool log, bool colz, double xmin, double xmax){
  ...
  if( !h1 ){
    std::cout<< std::endl;
    std::cout<< "Object doesn't found: "<< nameHisto << std::endl;
    return;
  } 
  if( !h2 ){
    std::cout<< std::endl;
  ...
  legend->SetFillStyle(0);
  legend->SetTextSize( 0.036);
  legend->AddEntry(h1, "L1HTT150", "LP");
  legend->AddEntry(h2, "HT450", "LP");
...
///////////////////////////////////////
void DrawComparison(TFile* dataFile1, TFile* dataFile2, const char *nameHisto, const char *XTitle, const char *YTitle, bool log, bool colz, double xmin, double xmax){
...
///////////////////////////////////////
void DrawComparison(TFile* dataFile, const char *nameHisto1, const char *nameHisto2, const char *XTitle, const char *YTitle, bool log, bool colz, double xmin, double xmax){

  TString HistoName1 =TString::Format(nameHisto1);
  TString HistoName2 =TString::Format(nameHisto2);
  double ymax;


  TH1F *h1 = (TH1F*)dataFile->Get(HistoName1 ); // Reco
  TH1F *h2 = (TH1F*)dataFile->Get(HistoName2); // HLT
  
  if( !h1 ){
    std::cout<< std::endl;
    std::cout<< "Histogram doesn't found: "<< nameHisto1 << std::endl;
    return;
  } 
  if( !h2 ){
    std::cout<< std::endl;
    std::cout<< "Histogram doesn't found: "<< nameHisto2 << std::endl;
    return;
  } 

  //  std::cout << HistoName1 << std::endl;
  //  std::cout << HistoName2 << std::endl;
  ...
  legend->SetFillStyle(0);
  legend->SetTextSize( 0.036);
  legend->AddEntry(h2, "HLT", "LP");
  legend->AddEntry(h1, "Reco", "LP");
  ...
  legend->Draw();
  canvas ->SaveAs("Plot/Compare_"+HistoName2+".png");
  std::cout<< "Saved: Compare_"<< HistoName2 << std::endl;
  if(pdfsave)  canvas ->SaveAs("Plot/Compare_"+HistoName2+".pdf");
  if(rootsave)  canvas ->SaveAs("Plot/Compare_"+HistoName2+".root");
  canvas ->Destructor();
}
/////////////////////////////////////
void DrawTProfile(TFile* dataFile, const char *nameProfile, const char *XTitle, const char *YTitle, bool log, double xmin, double xmax){
  ...
  if( !p1 ){
    std::cout<< std::endl;
    std::cout<< "TProfile doesn't found: "<< nameProfile<< std::endl;
    return;
  } 
  ...
  p1    -> SetXTitle(XTitle);
  p1    -> SetYTitle(YTitle);
  p1    -> GetYaxis()->SetTitleOffset(1.7);
  p1    -> GetXaxis()->SetRangeUser(xmin, xmax);
  //p1    -> GetYaxis()->SetRangeUser(-0.1, 0.1);
  p1    -> Draw();
  canvas->SetLeftMargin(0.13);
  canvas ->SaveAs("Plot/"+ProfileName+".png");
  std::cout<< "Saved: "<< ProfileName << std::endl;
  if(pdfsave)  canvas ->SaveAs("Plot/"+ProfileName+".pdf");
  if(rootsave)  canvas ->SaveAs("Plot/"+ProfileName+".root");
  canvas ->Destructor();
}
...
void DrawCompareProfile(TFile* dataFile, const char *nameProfile, const char *XTitle, const char *YTitle, bool log, bool colz, double xmin, double xmax){
  ...
  if( !p1 ){
    std::cout<< std::endl;
    std::cout<< "TProfile doesn't found: "<< nameProfile <<"HLT"<< std::endl;
    return;
  } 
  if( !p2 ){
    std::cout<< std::endl;
    std::cout<< "TProfile doesn't found: "<< nameProfile <<"Reco"<< std::endl;
    return;
  } 
  ...
  std::cout<< "Saved: Compare_"<< ProfileName << std::endl;
  if(pdfsave)  canvas ->SaveAs("Plot/Compare_"+ProfileName+".pdf");
  if(rootsave)  canvas ->SaveAs("Plot/Compare_"+ProfileName+".root");
  canvas ->Destructor();
}
