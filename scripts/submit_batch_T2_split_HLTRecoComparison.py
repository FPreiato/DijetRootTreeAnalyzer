...
usage = "usage: To be run from DijetRootTreeAnalyzer/ :  python scripts/submit_batch_T2_split_HLTRecoComparison.py -q cmslong -i lists_dir -m name_list -o output_dir --split 10 --tag TAGNAME -c config/cutFile_HLTRecoComparison.txt"
...
for line in  ins:
  ...
  for jj in range(0,njobs_list[i_f]+1):
    ...
    command = "./main "+splittedlist[jj]+" batch/"+cutfileName+" rootTupleTree/tree /tmp/rootfile_"+sample+"_"+newTag+"_"+str(jj)+" /tmp/cutEfficiencyFile_"+sample+"_"+newTag+"_"+str(jj)
