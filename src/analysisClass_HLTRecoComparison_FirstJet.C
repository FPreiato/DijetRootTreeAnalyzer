...
void analysisClass::Loop()
{
   ...
   TH1F* h_pTRecoJet_Binned = new TH1F("pTRecoJet_Binned","pTRecoJet", binnum, ptBins);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) { // running over all events
     ...
     if( PassJSON){
       ...
       CreateAndFillUserTH1D("mJJ_NoTrigger", 300, 0, 3000, mjjreco); 
	 ...
	 CreateAndFillUserTH1D("mJJ_passTriggers", 300, 0, 3000, mjjreco); 
	 ...
	 CreateAndFillUserTH1D("mJJ_passHLT_ZeroBias", 300, 0, 3000, mjjreco); 
	 ...
	 CreateAndFillUserTH1D("mJJ_passHLT_L1DoubleMu", 300, 0, 3000, mjjreco); 
	 ...
	 CreateAndFillUserTH1D("mJJ_passHLT_CaloJet40", 300, 0, 3000, mjjreco); 
	 ...
	 CreateAndFillUserTH1D("mJJ_passHLT_L1HTT150", 300, 0, 3000, mjjreco); 
	 ...
	 CreateAndFillUserTH1D("mJJ_passHLT_CaloScoutingHT250", 300, 0, 3000, mjjreco); 
	 ...
	 CreateAndFillUserTH1D("mJJ_passHLT_HT450", 300, 0, 3000, mjjreco); 
	 CreateAndFillUserTH1D("deltaEtaJJ_passHLT_HT450", 100, 0, 3, deltaETAjjreco); 
       }
       // #7
       CreateAndFillUserTH1D("passHLT_PFHT800", 2 , -0.5, 1.5, passHLT_PFHT800);
       if( passHLT_PFHT800 ){
	 CreateAndFillUserTH1D("mJJ_passHLT_PFHT800", 300, 0, 3000, mjjreco); 
	 CreateAndFillUserTH1D("deltaEtaJJ_passHLT_PFHT800", 100, 0, 3, deltaETAjjreco); 
       }
       // #8
       CreateAndFillUserTH1D("passHLT_PFHT650MJJ950", 2 , -0.5, 1.5, passHLT_PFHT650MJJ950);
       if( passHLT_PFHT650MJJ950 ){
	 CreateAndFillUserTH1D("mJJ_passHLT_PFHT650MJJ950", 300, 0, 3000, mjjreco); 
	 CreateAndFillUserTH1D("deltaEtaJJ_passHLT_PFHT650MJJ950", 100, 0, 3, deltaETAjjreco); 
       }
       // #9
       CreateAndFillUserTH1D("passHLT_PFHT650MJJ900", 2 , -0.5, 1.5, passHLT_PFHT650MJJ900);
       if( passHLT_PFHT650MJJ900 ){
	 CreateAndFillUserTH1D("mJJ_passHLT_PFHT650MJJ900", 300, 0, 3000, mjjreco); 
	 CreateAndFillUserTH1D("deltaEtaJJ_passHLT_PFHT650MJJ900", 100, 0, 3, deltaETAjjreco); 
       }
       //////////////////// end of trigger study
       //  if( passHLT_HT450_BtagSeq || passHLT_HT450 ){ // trigger tight
       //  if( mjjreco > MJJCut_ ){ // cut only on mJJ Reco	  
       //  if( passHLT_ZeroBias_BtagSeq || passHLT_ZeroBias || passHLT_L1DoubleMu_BtagSeq || passHLT_L1DoubleMu || passHLT_CaloJet40_BtagSeq || passHLT_CaloJet40 || passHLT_L1HTT150_BtagSeq || passHLT_L1HTT150 || passHLT_CaloScoutingHT250 || passHLT_HT450_BtagSeq || passHLT_HT450 || passHLT_PFHT800 || passHLT_PFHT650MJJ950 || passHLT_PFHT650MJJ900 ){       
       // trigger selected
       if( passHLT_HT450_BtagSeq || passHLT_HT450 ){       
	 if( deltaETAjjreco < DeltaEtaJJ_){ // cut only on Reco
	   
	   CreateAndFillUserTH1D("AK4_HTReco", 200 , 0, 2000, htAK4reco);
	   CreateAndFillUserTH1D("AK4_HTHLT", 200 , 0, 2000, htAK4);
	   CreateAndFillUserTH2D("AK4_HT", 200, 0, 2000, 200, 0, 2000, htAK4reco, htAK4);
	   ...
	   if( AK4Comparison ){
	     ...
	     if( DeltaReco1HLT1 <= DeltaReco1HLT2 )
	       {
		 IdxMatched =  0 ; // index HLT jets
		 deltaR_min   = DeltaReco1HLT1;
		 deltaR_min2 = DeltaReco2HLT2;
	       }else
	       {
		 IdxMatched = 1 ; // index HLT jets
		 deltaR_min   = DeltaReco1HLT2;
		 deltaR_min2 = DeltaReco2HLT1;
	       }

	     // HLT-Reco Matched : AK4
	     if(verbose) cout << "deltaR_min "   << deltaR_min    << endl;
	     if(verbose) cout << "deltaR_min2 " << deltaR_min2  << endl;
	     CreateAndFillUserTH1D("AK4_deltaR_minimum", 500, 0, 5, deltaR_min );
	     
	     if(deltaR_min < DeltaR_ ){   
	       CreateAndFillUserTH1D("AK4_deltaR_minimum_AfterMatch", 500, 0, 5, deltaR_min);
	       if(verbose){
		 std::cout<<"AK4"<<std::endl;
		 std::cout<<"First jet Reco"<<std::endl;
		 std::cout<<"Pt= "<< AK4recojets[0].Pt()<<" Eta= "<<AK4recojets[0].Eta()<< " Phi= "<<AK4recojets[0].Phi()  <<std::endl;
		 std::cout<<"HLT" <<std::endl;
		 std::cout<<"Index= " << IdxMatched <<std::endl;
		 std::cout<<"Pt= "<< AK4jets[IdxMatched].Pt()<<" Eta= "<<AK4jets[IdxMatched].Eta()<< " Phi= "<<AK4jets[IdxMatched].Phi()  <<std::endl;
	       }
	       
	       h_pTRecoJet_Binned -> Fill( AK4recojets[0].Pt() );	       

	       // Reco variables
	       CreateAndFillUserTH1D("AK4_pT_JetReco", 300, 0, 3000, AK4recojets[0].Pt() );
	       CreateAndFillUserTH1D("AK4_Eta_JetReco", 100, -2.5, 2.5, AK4recojets[0].Eta() );
	       CreateAndFillUserTH1D("AK4_Phi_JetReco", 100, -3.15, 3.15, AK4recojets[0].Phi() );
	       CreateAndFillUserTH1D("AK4_chargedHadEnFracReco", 100, 0, 1, chargedHadEnFracReco[0] );
	       CreateAndFillUserTH1D("AK4_neutrHadEnFracReco", 100, 0, 1, neutrHadEnFracReco[0] );
	       CreateAndFillUserTH1D("AK4_chargedElectromFracReco", 100, 0, 1, chargedElectromFracReco[0] );
	       CreateAndFillUserTH1D("AK4_neutrElectromFracReco", 100, 0, 1, neutrElectromFracReco[0] );
	       CreateAndFillUserTH1D("AK4_muEnFracReco", 100, 0, 1, muEnFracReco[0] );
	       CreateAndFillUserTH1D("AK4_chargedMultReco", 100, 0, 100, chargedMultReco[0] );
	       CreateAndFillUserTH1D("AK4_neutrMultReco", 100, 0, 100, neutrMultReco[0] );
	       CreateAndFillUserTH1D("AK4_photonMultReco", 100, 0, 100, photonMultReco[0] );
	       CreateAndFillUserTH1D("AK4_jetCSVAK4Reco", 100, 0, 2, jetCSVAK4Reco[0] );
	       // HLT variables
	       CreateAndFillUserTH1D("AK4_pT_JetHLT", 300, 0, 3000, AK4jets[IdxMatched].Pt() );
	       CreateAndFillUserTH1D("AK4_Eta_JetHLT", 100, -2.5, 2.5, AK4jets[IdxMatched].Eta() );
	       CreateAndFillUserTH1D("AK4_Phi_JetHLT", 100, -3.15, 3.15, AK4jets[IdxMatched].Phi() );
	       CreateAndFillUserTH1D("AK4_chargedHadEnFracHLT", 100, 0, 1, chargedHadEnFrac[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_neutrHadEnFracHLT", 100, 0, 1, neutrHadEnFrac[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_chargedElectromFracHLT", 100, 0, 1, chargedElectromFrac[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_neutrElectromFracHLT", 100, 0, 1, neutrElectromFrac[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_muEnFracHLT", 100, 0, 1, muEnFrac[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_chargedMultHLT", 100, 0, 100, chargedMult[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_neutrMultHLT", 100, 0, 100, neutrMult[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_photonMultHLT", 100, 0, 100, photonMult[IdxMatched] );
	       CreateAndFillUserTH1D("AK4_jetCSVAK4HLT", 100, 0, 2, jetCSVAK4[IdxMatched] );
	       // Reco vs HLT
	       CreateAndFillUserTH2D("AK4_pT_Jet", 300, 0, 3000, 300, 0, 3000, AK4recojets[0].Pt(), AK4jets[IdxMatched].Pt() );
	       CreateAndFillUserTH2D("AK4_Eta_Jet", 100, -2.5, 2.5, 100, -2.5, 2.5, AK4recojets[0].Eta(), AK4jets[IdxMatched].Eta() );
	       CreateAndFillUserTH2D("AK4_Phi_Jet", 100, -3.15, 3.15, 100, -3.15, 3.15, AK4recojets[0].Phi(), AK4jets[IdxMatched].Phi() );
	       CreateAndFillUserTH2D("AK4_chargedHadEnFrac", 100, 0, 1, 100, 0, 1, chargedHadEnFracReco[0], chargedHadEnFrac[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_neutrHadEnFrac", 100, 0, 1, 100, 0, 1, neutrHadEnFracReco[0], neutrHadEnFrac[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_chargedElectromFrac", 100, 0, 1, 100, 0, 1, chargedElectromFracReco[0], chargedElectromFrac[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_neutrElectromFrac", 100, 0, 1, 100, 0, 1, neutrElectromFracReco[0], neutrElectromFrac[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_muEnFrac", 100, 0, 1, 100, 0, 1, muEnFracReco[0], muEnFrac[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_chargedMult", 100, 0, 100, 100, 0, 100, chargedMultReco[0], chargedMult[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_neutrMult", 100, 0, 100, 100, 0, 100, neutrMultReco[0], neutrMult[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_photonMult", 100, 0, 100, 100, 0, 100, photonMultReco[0], photonMult[IdxMatched] );
	       CreateAndFillUserTH2D("AK4_jetCSVAK4", 100, 0, 2, 100, 0, 2, jetCSVAK4Reco[0], jetCSVAK4[IdxMatched] );
	     
	       // Bias in pT AK4
	       // pT(HLT) - pT(Reco) / pT(Reco)	   
	       double pTBias = (AK4jets[IdxMatched].Pt() - AK4recojets[0].Pt() ) / AK4recojets[0].Pt();
	       if(verbose) std::cout<<"pT Bias= "<<pTBias << std::endl;

	       int AK4_etaBin = mEtaBinning.getBin( fabs(AK4recojets[0].Eta()) );
	       int AK4_ptBin   = mPtBinning.getPtBin( AK4recojets[0].Pt() );
	       ...
	       if( verbose ){	       
		 cout<<"AK4: |Eta| =   "<< fabs( AK4recojets[0].Eta() ) << " pT = "<< AK4recojets[0].Pt() <<endl;
		 ...
		 cout<<"AK4 Bin  "  << AK4_Bin.c_str() <<endl;
	       }

	       CreateAndFillUserTH1D("AK4_pTBias", 300, -2., 2., pTBias );
	       CreateAndFillUserTH1D( Form("AK4_pTBias_%s", AK4_Bin.c_str()), 300, -2., 2., pTBias );

	       // bias vs reco quantity
	       ////// TProfile: pTBias vs nVtx
	       CreateAndFillUserTProfile("AK4_pTBias_vs_Nvtx",  50, 0., 50., -2., 2.,      nVtxreco, pTBias );
	       CreateAndFillUserTProfile("AK4_pT_vs_NvtxReco", 50, 0., 50., 0., 5000., nVtxreco, AK4recojets[0].Pt() );
	       CreateAndFillUserTProfile("AK4_pT_vs_NvtxHLT",  50, 0., 50., 0., 5000., nVtx, AK4jets[IdxMatched].Pt() );
	       CreateAndFillUserTProfile( Form("AK4_pTBias_vs_Nvtx_%s", AK4_Bin.c_str()),  50, 0., 50., -2., 2.,      nVtxreco, pTBias );
	       CreateAndFillUserTProfile( Form("AK4_pT_vs_NvtxReco_%s", AK4_Bin.c_str()), 50, 0., 50., 0., 5000., nVtxreco, AK4recojets[0].Pt() );
	       CreateAndFillUserTProfile( Form("AK4_pT_vs_NvtxHLT_%s", AK4_Bin.c_str()),  50, 0., 50., 0., 5000., nVtx, AK4jets[IdxMatched].Pt() );
	       ////// TProfile: pTBias vs Eta
	       CreateAndFillUserTProfile("AK4_pTBias_vs_Eta",  50, -2.5, 2.5, -2., 2.,      AK4recojets[0].Eta(), pTBias );
	       CreateAndFillUserTProfile("AK4_pT_vs_EtaReco", 50, -2.5, 2.5, 0., 5000., AK4recojets[0].Eta(), AK4recojets[0].Pt() );
	       CreateAndFillUserTProfile("AK4_pT_vs_EtaHLT",  50, -2.5, 2.5, 0., 5000., AK4jets[IdxMatched].Eta(), AK4jets[IdxMatched].Pt() );
	       CreateAndFillUserTProfile( Form("AK4_pTBias_vs_Eta_%s", AK4_Bin.c_str()),  50, -2.5, 2.5, -2., 2.,      AK4recojets[0].Eta(), pTBias );
	       CreateAndFillUserTProfile( Form("AK4_pT_vs_EtaReco_%s", AK4_Bin.c_str()), 50, -2.5, 2.5, 0., 5000., AK4recojets[0].Eta(), AK4recojets[0].Pt() );
	       CreateAndFillUserTProfile( Form("AK4_pT_vs_EtaHLT_%s", AK4_Bin.c_str()),  50, -2.5, 2.5, 0., 5000., AK4jets[IdxMatched].Eta(), AK4jets[IdxMatched].Pt() );
	       ////// TProfile: pTBias vs Pt
	       CreateAndFillUserTProfile("AK4_pTBias_vs_Pt",  500, 0., 5000., -2., 2.,      AK4recojets[0].Pt(), pTBias );
	       CreateAndFillUserTProfile("AK4_pT_vs_PtReco", 500, 0., 5000., 0., 5000., AK4recojets[0].Pt(), AK4recojets[0].Pt() );
	       CreateAndFillUserTProfile("AK4_pT_vs_PtHLT",  500, 0., 5000., 0., 5000., AK4jets[IdxMatched].Pt(), AK4jets[IdxMatched].Pt() );	      
	       CreateAndFillUserTProfile( Form("AK4_pTBias_vs_Pt_%s", AK4_Bin.c_str()),  500, 0., 5000., -2., 2.,      AK4recojets[0].Pt(), pTBias );
	       CreateAndFillUserTProfile( Form("AK4_pT_vs_PtReco_%s", AK4_Bin.c_str()), 500, 0., 5000., 0., 5000., AK4recojets[0].Pt(), AK4recojets[0].Pt() );
	       CreateAndFillUserTProfile( Form("AK4_pT_vs_PtHLT_%s", AK4_Bin.c_str()),  500, 0., 5000., 0., 5000., AK4jets[IdxMatched].Pt(), AK4jets[IdxMatched].Pt() );	      

	     }//if matching

	     // only for AK4: jets Asymmetry --> Resolution
	     // Asymmetry only for events with 2 matched jets (?) 
	     if(deltaR_min < DeltaR_ && deltaR_min2 < DeltaR_ ){   
	       int random;
	       random = lrand48()%(2);
	       ...
	       CreateAndFillUserTH1D("pTaveReco", 300 , 0, 3000, pTaveReco);
		 ...
		 cout<<"ptBin.first  "<< ptBins_Reco.first << "    ptBin.second   "<<ptBins_Reco.second<<endl;
	       ...
	       if(etaBin_j1Reco == etaBin_j2Reco){
		 std::string etaName = mEtaBinning.getBinName(etaBin_j1Reco);	     
		 if(alphaReco < alphacut_ ){	
		   if(random == 0){
		   ...
		   std::string HistoName = TString::Format("AsymmetryReco_%s_pT_%i_%i", etaName.c_str(), (int) ptBins_Reco.first, (int) ptBins_Reco.second ).Data();	 	     
		   if(verbose) std::cout << HistoName.c_str()<< std::endl;
		   if(verbose) std::cout<< "Asymmetry Reco = " << AsymmetryReco << std::endl; 
		   CreateAndFillUserTH1D(HistoName.c_str(), 300, -2, 2, AsymmetryReco);
		 }else{//alpha cut
	   ...
	   if(WJComparison){
		 ...
		 std::cout<<"Pt= "<< widejetsReco[0].Pt()<<" Eta= "<<widejetsReco[0].Eta()<< " Phi= "<<widejetsReco[0].Phi()  <<std::endl;
		 std::cout<<"HLT" <<std::endl;
		 std::cout<<"Index= "<< IdxMatched.at(0)  <<std::endl;
		 std::cout<<"Pt= "<< widejets[IdxMatched.at(0)].Pt()<<" Eta= "<<widejets[IdxMatched.at(0)].Eta()<< " Phi= "<<widejets[IdxMatched.at(0)].Phi()  <<std::endl;
	       }
	       
	       // Reco variables
	       CreateAndFillUserTH1D("pT_WideJetReco", 300, 0, 3000, widejetsReco[0].Pt()  );
	       CreateAndFillUserTH1D("Eta_WideJetReco", 100, -2.5, 2.5, widejetsReco[0].Eta() );
	       CreateAndFillUserTH1D("Phi_WideJetReco", 100, -3.5, 3.5, widejetsReco[0].Phi()  );
	       // HLT variables	       
	       CreateAndFillUserTH1D("pT_WideJetHLT", 300, 0, 3000, widejets[IdxMatched.at(0)].Pt()  );
	       CreateAndFillUserTH1D("Eta_WideJetHLT", 100, -2.5, 2.5, widejets[IdxMatched.at(0)].Eta()  );
	       CreateAndFillUserTH1D("Phi_WideJetHLT", 100, -3.5, 3.5, widejets[IdxMatched.at(0)].Phi()  );
	       // Reco vs HLT
	       CreateAndFillUserTH2D("pT_WideJet", 300, 0, 3000, 300, 0, 3000, widejetsReco[0].Pt(), widejets[IdxMatched.at(0)].Pt() );
	       CreateAndFillUserTH2D("Eta_WideJet", 100, -2.5, 2.5, 100, -2.5, 2.5, widejetsReco[0].Eta(), widejets[IdxMatched.at(0)].Eta() );
	       CreateAndFillUserTH2D("Phi_WideJet", 100, -3.5, 3.5, 100, -3.5, 3.5, widejetsReco[0].Phi(), widejets[IdxMatched.at(0)].Phi() );
	       
	       double pTBias = ( widejets[IdxMatched.at(0)].Pt() - widejetsReco[0].Pt() )/ widejetsReco[0].Pt() ; 
	       if(verbose) std::cout<<"pT Bias= "<<pTBias << std::endl;
	       
	       int etaBin = mEtaBinning.getBin( fabs(widejetsReco[0].Eta()) );
	       int ptBin   = mPtBinning.getPtBin( widejetsReco[0].Pt() );
	       std::pair<float, float> ptBins = mPtBinning.getBinValue( ptBin );
	       ...
	       if( verbose ){	       
		 cout<<"WideJet: |Eta| = "<< fabs( widejetsReco[0].Eta() )<< " pT = "<< widejetsReco[0].Pt() <<endl;
		 ...
		 cout<<"Bin  "  << Bin.c_str() <<endl;
	       ...
	       ////// TProfile: pTBias vs nVtx
	       CreateAndFillUserTProfile("pTBias_vs_Nvtx",  50, 0., 50., -2., 2.,      nVtxreco, pTBias );
	       CreateAndFillUserTProfile("pT_vs_NvtxReco", 50, 0., 50., 0., 5000., nVtxreco, widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile("pT_vs_NvtxHLT",  50, 0., 50., 0., 5000., nVtx, widejets[IdxMatched.at(0)].Pt() );
	       CreateAndFillUserTProfile( Form("pTBias_vs_Nvtx_%s", Bin.c_str()),  50, 0., 50., -2., 2.,      nVtxreco, pTBias );
	       CreateAndFillUserTProfile( Form("pT_vs_NvtxReco_%s", Bin.c_str()), 50, 0., 50., 0., 5000., nVtxreco, widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile( Form("pT_vs_NvtxHLT_%s", Bin.c_str()),  50, 0., 50., 0., 5000., nVtx, widejets[IdxMatched.at(0)].Pt() );
	       ////// TProfile: pTBias vs Eta
	       CreateAndFillUserTProfile("pTBias_vs_Eta",  50, -2.5, 2.5, -2., 2.,      widejetsReco[0].Eta(), pTBias );
	       CreateAndFillUserTProfile("pT_vs_EtaReco", 50, -2.5, 2.5, 0., 5000., widejetsReco[0].Eta(), widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile("pT_vs_EtaHLT",  50, -2.5, 2.5, 0., 5000., widejets[IdxMatched.at(0)].Eta(), widejets[IdxMatched.at(0)].Pt() );
	       CreateAndFillUserTProfile( Form("pTBias_vs_Eta_%s", Bin.c_str()),  50, -2.5, 2.5, -2., 2.,      widejetsReco[0].Eta(), pTBias );
	       CreateAndFillUserTProfile( Form("pT_vs_EtaReco_%s", Bin.c_str()), 50, -2.5, 2.5, 0., 5000., widejetsReco[0].Eta(), widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile( Form("pT_vs_EtaHLT_%s", Bin.c_str()),  50, -2.5, 2.5, 0., 5000., widejets[IdxMatched.at(0)].Eta(), widejets[IdxMatched.at(0)].Pt() );
	       ////// TProfile: pTBias vs Pt
	       CreateAndFillUserTProfile("pTBias_vs_Pt",  500, 0., 5000., -2., 2.,      widejetsReco[0].Pt(), pTBias );
	       CreateAndFillUserTProfile("pT_vs_PtReco", 500, 0., 5000., 0., 5000., widejetsReco[0].Pt(), widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile("pT_vs_PtHLT",  500, 0., 5000., 0., 5000., widejets[IdxMatched.at(0)].Pt(), widejets[IdxMatched.at(0)].Pt() );	      
	       CreateAndFillUserTProfile( Form("pTBias_vs_Pt_%s", Bin.c_str()),  500, 0., 5000., -2., 2.,      widejetsReco[0].Pt(), pTBias );
	       CreateAndFillUserTProfile( Form("pT_vs_PtReco_%s", Bin.c_str()), 500, 0., 5000., 0., 5000., widejetsReco[0].Pt(), widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile( Form("pT_vs_PtHLT_%s", Bin.c_str()),  500, 0., 5000., 0., 5000., widejets[IdxMatched.at(0)].Pt(), widejets[IdxMatched.at(0)].Pt() );	      
	       // non sono sicuro vada qui	     
	       ////// TProfile: pTBias vs mJJ
	       CreateAndFillUserTProfile("pTBias_vs_Mjj",  500, 0., 5000., -2., 2.,      mjjreco, pTBias );
	       CreateAndFillUserTProfile("pT_vs_MjjReco", 500, 0., 5000., 0., 5000., mjjreco, widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile("pT_vs_MjjHLT",  500, 0., 5000., 0., 5000., mjj, widejets[IdxMatched.at(0)].Pt() );	      
	       CreateAndFillUserTProfile( Form("pTBias_vs_Mjj_%s", Bin.c_str()),  500, 0., 5000., -2., 2.,      mjjreco, pTBias );
	       CreateAndFillUserTProfile( Form("pT_vs_MjjReco_%s", Bin.c_str()), 500, 0., 5000., 0., 5000., mjjreco, widejetsReco[0].Pt() );
	       CreateAndFillUserTProfile( Form("pT_vs_MjjHLT_%s", Bin.c_str()),  500, 0., 5000., 0., 5000., mjj, widejets[IdxMatched.at(0)].Pt() );	      
	     ...
	     if(deltaR_min.at(0) < DeltaR_ && deltaR_min.at(1) < DeltaR_){
	       ...
	       // mJJ(HLT) - mJJ(Reco) / mJJ(Reco)
	       double mJJBias =  (mjj - mjjreco) / mjjreco;
	       if(verbose) std::cout<<"mJJ Bias= "<<mJJBias << std::endl;
	       
	       int MjjBin   = mMjjBinning.getBin( mjjreco );
		 ...
		 cout<<"mJJBin  "  << mJJBin.c_str() <<endl;
	       ...
	       CreateAndFillUserTProfile("mJJBias_vs_Nvtx",  50, 0., 50., -2., 2.,      nVtxreco, mJJBias );
	       ...
	       CreateAndFillUserTProfile( Form("mJJBias_vs_Nvtx_%s", mJJBin.c_str()),  50, 0., 50., -2., 2.,      nVtxreco, mJJBias );
	       ...
	       CreateAndFillUserTProfile("mJJBias_vs_Eta",  50, -2.5, 2.5, -2., 2.,      widejetsReco[0].Eta(), mJJBias );
	       CreateAndFillUserTProfile("mJJ_vs_EtaReco", 50, -2.5, 2.5, 0., 5000., widejetsReco[0].Eta(), mjjreco );
	       CreateAndFillUserTProfile("mJJ_vs_EtaHLT",  50, -2.5, 2.5, 0., 5000., widejets[IdxMatched.at(0)].Eta(), mjj );
	       CreateAndFillUserTProfile( Form("mJJBias_vs_Eta_%s", mJJBin.c_str()),  50, -2.5, 2.5, -2., 2.,      widejetsReco[0].Eta(), mJJBias );
	       CreateAndFillUserTProfile( Form("mJJ_vs_EtaReco_%s", mJJBin.c_str()), 50, -2.5, 2.5, 0., 5000., widejetsReco[0].Eta(), mjjreco );
	       CreateAndFillUserTProfile( Form("mJJ_vs_EtaHLT_%s", mJJBin.c_str()),  50, -2.5, 2.5, 0., 5000., widejets[IdxMatched.at(0)].Eta(), mjj );
	       ////// TProfile: mJJBias vs Pt
	       CreateAndFillUserTProfile("mJJBias_vs_Pt",  500, 0., 5000., -2., 2.,      widejetsReco[0].Pt(), mJJBias );
	       CreateAndFillUserTProfile("mJJ_vs_PtReco", 500, 0., 5000., 0., 5000., widejetsReco[0].Pt(), mjjreco );
	       CreateAndFillUserTProfile("mJJ_vs_PtHLT",  500, 0., 5000., 0., 5000., widejets[IdxMatched.at(0)].Pt(), mjj );	      
	       CreateAndFillUserTProfile( Form("mJJBias_vs_Pt_%s", mJJBin.c_str()),  500, 0., 5000., -2., 2.,      widejetsReco[0].Pt(), mJJBias );
	       CreateAndFillUserTProfile( Form("mJJ_vs_PtReco_%s", mJJBin.c_str()), 500, 0., 5000., 0., 5000., widejetsReco[0].Pt(), mjjreco );
	       CreateAndFillUserTProfile( Form("mJJ_vs_PtHLT_%s", mJJBin.c_str()),  500, 0., 5000., 0., 5000., widejets[IdxMatched.at(0)].Pt(), mjj );	      
	       ////// TProfile: mJJBias vs mJJ
	       CreateAndFillUserTProfile("mJJBias_vs_Mjj",  500, 0., 5000., -2., 2.,      mjjreco, mJJBias );
	       CreateAndFillUserTProfile("mJJ_vs_MjjReco", 500, 0., 5000., 0., 5000., mjjreco, mjjreco );
	       CreateAndFillUserTProfile("mJJ_vs_MjjHLT",  500, 0., 5000., 0., 5000., mjj, mjj );	      
	       CreateAndFillUserTProfile( Form("mJJBias_vs_Mjj_%s", mJJBin.c_str()),  500, 0., 5000., -2., 2.,      mjjreco, mJJBias );
	       CreateAndFillUserTProfile( Form("mJJ_vs_MjjReco_%s", mJJBin.c_str()), 500, 0., 5000., 0., 5000., mjjreco, mjjreco );
	       CreateAndFillUserTProfile( Form("mJJ_vs_MjjHLT_%s", mJJBin.c_str()),  500, 0., 5000., 0., 5000., mjj, mjj );	      
